  ...
  return (
    <div className="fixed w-64">
      <div className=" hover:bg-white bg-gray-50 transition-all flex group items-center w-full overflow-hidden rounded-xl text-left shadow-md">
        ...
        <input 
          data-testid='input'
          className="outline-none border-none py-2 pl-3 pr-10 leading-5 hover:bg-white bg-gray-50 transition-all text-gray-800 focus:ring-0"
          type="text"
        ...
        <div className="relative cursor-default select-none text-gray-700 rounded-xl">
          {filterOptions().length === 0 ? (
            ...
            filterOptions().map((option, index) => (
              <div 
                className = 
                  {`flex px-2 py-2 hover:bg-teal-700 hover:text-white ${index != 0 && 'border-t'}`}
                key={option.id}
